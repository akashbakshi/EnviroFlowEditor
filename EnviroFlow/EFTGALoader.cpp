...
GLuint LoadTGATexture(char *TexName)
  {
   TGAImg tgaImg;        // Image loader


   GLuint Texture;
 
  // Load our Texture
   if(tgaImg.Load(TexName)!=IMG_OK)
    return -1;
   ...
   // Create the texture
    if(tgaImg.GetBPP()==24)
     glTexImage2D(GL_TEXTURE_2D,0,3,tgaImg.GetWidth(),tgaImg.GetHeight(),0,
                  GL_RGB,GL_UNSIGNED_BYTE,tgaImg.GetImg());
    else if(tgaImg.GetBPP()==32)
     glTexImage2D(GL_TEXTURE_2D,0,4,tgaImg.GetWidth(),tgaImg.GetHeight(),0,
                  GL_RGBA,GL_UNSIGNED_BYTE,tgaImg.GetImg());
